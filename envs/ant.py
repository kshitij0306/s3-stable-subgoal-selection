...
class AntEnv(mujoco_env.MujocoEnv, utils.EzPickle):
  ...
  def viewer_setup(self):
    ...
    # Keep a free camera so we can bias the framing towards the maze layout.
    cam.trackbodyid = -1

    extent = float(getattr(self.model.stat, 'extent', 0.0) or 0.0)
    center = np.zeros(3, dtype=np.float32)
    if hasattr(self.model.stat, 'center'):
      center = np.array(self.model.stat.center, dtype=np.float32)

    cam.distance = max(extent * 1.45, 10.0)
    cam.elevation = -42
    cam.azimuth = 90
    cam.lookat[0] = center[0]
    cam.lookat[1] = center[1] + 0.0 * extent
    # cam.lookat[2] = max(center[2] + 1.3, 1.3)
    cam.lookat[2] = center[2]
