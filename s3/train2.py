...
def run_s3(args):
    ...
    while total_timesteps < args.max_timesteps:
        ...
        ext_reward = manager_reward * args.man_rew_scale
        manager_reward = manager_reward * (1 - man_ctrl_rew_balance) + controller_reward * (man_ctrl_rew_balance)
...
def anneal_balance(step: int, start: float, min_b: float, decay_steps: int) -> float:
    if decay_steps <= 0:                 # disable
        return start
    frac = max(min_b, start - step / decay_steps)
    return frac
